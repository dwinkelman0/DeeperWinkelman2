...
/*
void GenerateRookAndKing() {
	TableBase tb;
	
	const char * king_pairs[40][2] = {
	...
	Board board;
	for (int i = 0; i < 40; i++) {
		int bking_square = Text2Coord(king_pairs[i][0]);
		int wking_square = Text2Coord(king_pairs[i][1]);
		for (int rook_square = 0; rook_square < 64; rook_square++) {
			BoardState state;
			state.white_OO = state.white_OOO = state.black_OO = state.black_OOO = false;
			state.ep_target = 0;
			state.white_to_move = false;
			state.squares[bking_square] = BLACK_KING;
			state.squares[wking_square] = WHITE_KING;
			state.squares[rook_square] = WHITE_ROOK;
			
			board.SetCurrent(state);
			if (board.IsCheckmate()) {
				tb.AddStaticallySolved(state, TableBase::Node::RESULT_WHITE_WIN);
			}
			else if (board.IsStalemate()) {
				tb.AddStaticallySolved(state, TableBase::Node::RESULT_DRAW);
			}
		}
	}
}
...
TableBase GenerateKings() {
	...
	// Remove frontier for compactness
	tb.ClearFrontier();
	return tb;
}
...
TableBase GenerateRookAndKing() {
	...
	for (int i_king = 0; i_king < 48; i_king++) {
		...
		for (; white_rook < white_rook_end; white_rook += white_rook_inc) {
			...
			if (valid) {
				tb.AddStaticallySolved(state, result);
			}
	...
	tb.AddStaticallySolved(state, res_draw);
		...
		for (int bking = 0; bking < 64; bking++) {
			...
			for (int wrook = 0; wrook < 64; wrook++) {
				...
				if (!board.InCheck(false)) {
					if (tb.Evaluate(state) == NULL) {
						tb.AddUnsolvedToFrontier(state);
					}
				}
